...
void VnUartPacket_parseVNYPR ( VnUartPacket* packet, vec3f* yawPitchRoll )
{
    ...
    yawPitchRoll->c[0] = ATOFF;
    NEXT
    yawPitchRoll->c[1] = ATOFF;
    NEXT
...
void VnUartPacket_parseVNQTN ( VnUartPacket* packet, vec4f* quaternion )
{
    ...
    quaternion->c[0] = ATOFF;
    NEXT
    quaternion->c[1] = ATOFF;
    NEXT
    quaternion->c[2] = ATOFF;
    NEXT
...
void VnUartPacket_parseVNQTA ( VnUartPacket* packet, vec4f* quaternion, vec3f* acceleration )
{
    ...
    quaternion->c[3] = ATOFF;
    NEXT
    acceleration->c[0] = ATOFF;
    NEXT
    acceleration->c[1] = ATOFF;
    NEXT
...
void VnUartPacket_parseVNQTR ( VnUartPacket* packet, vec4f* quaternion, vec3f* angularRate )
{
    ...
    quaternion->c[3] = ATOFF;
    NEXT
    angularRate->c[0] = ATOFF;
    NEXT
    angularRate->c[1] = ATOFF;
    NEXT
...
void VnUartPacket_parseVNYBA ( VnUartPacket* packet, vec3f* yawPitchRoll, vec3f* accelerationBody, vec3f* angularRate )
{
    ...
    yawPitchRoll->c[2] = ATOFF;
    NEXT
    accelerationBody->c[0] = ATOFF;
    NEXT
    accelerationBody->c[1] = ATOFF;
    NEXT
    accelerationBody->c[2] = ATOFF;
    NEXT
    angularRate->c[0] = ATOFF;
    NEXT
    angularRate->c[1] = ATOFF;
...
void VnUartPacket_parseVNYIA ( VnUartPacket* packet, vec3f* yawPitchRoll, vec3f* accelerationInertial, vec3f* angularRate )
{
    ...
    yawPitchRoll->c[2] = ATOFF;
    NEXT
    accelerationInertial->c[0] = ATOFF;
    NEXT
    accelerationInertial->c[1] = ATOFF;
    NEXT
    accelerationInertial->c[2] = ATOFF;
    NEXT
    angularRate->c[0] = ATOFF;
    NEXT
    angularRate->c[1] = ATOFF;
...
void VnUartPacket_parseVNIMU ( VnUartPacket* packet, vec3f* magneticUncompensated, vec3f* accelerationUncompensated, vec3f* angularRateUncompensated, float* temperature, float* pressure )
{
    ...
    magneticUncompensated->c[0] = ATOFF;
    NEXT
    magneticUncompensated->c[1] = ATOFF;
    NEXT
    magneticUncompensated->c[2] = ATOFF;
    NEXT
    accelerationUncompensated->c[0] = ATOFF;
    NEXT
    accelerationUncompensated->c[1] = ATOFF;
    NEXT
    accelerationUncompensated->c[2] = ATOFF;
    NEXT
    angularRateUncompensated->c[0] = ATOFF;
    NEXT
    angularRateUncompensated->c[1] = ATOFF;
    NEXT
    angularRateUncompensated->c[2] = ATOFF;
    NEXT
    *temperature = ATOFF;
    NEXT
    *pressure = ATOFF;
...
void VnUartPacket_parseVNINS ( VnUartPacket* packet, double* time, uint16_t* week, uint16_t* status, vec3f* yawPitchRoll, vec3d* lla, vec3f* nedVel, float* attUncertainty, float* posUncertainty, float* velUncertainty )
{
    ...
    *status = ATOU16X;
    ...
    nedVel->c[2] = ATOFF;
    NEXT
    *attUncertainty = ATOFF;
    NEXT
    *posUncertainty = ATOFF;
    NEXT
...
void VnUartPacket_parseVNRAW ( VnUartPacket* packet, vec3f *magneticVoltage, vec3f *accelerationVoltage, vec3f *angularRateVoltage, float* temperatureVoltage )
{
    ...
    magneticVoltage->c[0] = ATOFF;
    NEXT
    magneticVoltage->c[1] = ATOFF;
    NEXT
    magneticVoltage->c[2] = ATOFF;
    NEXT
    accelerationVoltage->c[0] = ATOFF;
    NEXT
    accelerationVoltage->c[1] = ATOFF;
    NEXT
    accelerationVoltage->c[2] = ATOFF;
    NEXT
    angularRateVoltage->c[0] = ATOFF;
    NEXT
    angularRateVoltage->c[1] = ATOFF;
    NEXT
    angularRateVoltage->c[2] = ATOFF;
    NEXT
...
void VnUartPacket_parseVNSTV ( VnUartPacket* packet, vec4f* quaternion, vec3f* angularRateBias )
{
    ...
    quaternion->c[3] = ATOFF;
    NEXT
    angularRateBias->c[0] = ATOFF;
    NEXT
    angularRateBias->c[1] = ATOFF;
    NEXT
...
void VnUartPacket_parseVNCOV ( VnUartPacket* packet, vec3f* attitudeVariance, vec3f* angularRateBiasVariance )
{
    ...
    attitudeVariance->c[0] = ATOFF;
    NEXT
    attitudeVariance->c[1] = ATOFF;
    NEXT
    attitudeVariance->c[2] = ATOFF;
    NEXT
    angularRateBiasVariance->c[0] = ATOFF;
    NEXT
    angularRateBiasVariance->c[1] = ATOFF;
    NEXT
...
void VnUartPacket_parseVNDTV ( VnUartPacket* packet, float* deltaTime, vec3f* deltaTheta, vec3f* deltaVelocity )
{
    ...
    *deltaTime = ATOFF;
    NEXT
    deltaTheta->c[0] = ATOFF;
    NEXT
    deltaTheta->c[1] = ATOFF;
    NEXT
    deltaTheta->c[2] = ATOFF;
    NEXT
    deltaVelocity->c[0] = ATOFF;
    NEXT
    deltaVelocity->c[1] = ATOFF;
    NEXT
...
void VnUartPacket_parseFilterMeasurementsVarianceParametersRaw ( char *packet, float* angularWalkVariance, vec3f* angularRateVariance, vec3f* magneticVariance, vec3f* accelerationVariance )
{
    ...
    *angularWalkVariance = ATOFF;
    NEXTRAW
    angularRateVariance->c[0] = ATOFF;
    NEXTRAW
    angularRateVariance->c[1] = ATOFF;
    NEXTRAW
    angularRateVariance->c[2] = ATOFF;
    NEXTRAW
    magneticVariance->c[0] = ATOFF;
    NEXTRAW
    magneticVariance->c[1] = ATOFF;
    NEXTRAW
    magneticVariance->c[2] = ATOFF;
    NEXTRAW
    accelerationVariance->c[0] = ATOFF;
    NEXTRAW
    accelerationVariance->c[1] = ATOFF;
    NEXTRAW
...
void VnUartPacket_parseFilterActiveTuningParametersRaw ( char *packet, float* magneticDisturbanceGain, float* accelerationDisturbanceGain, float* magneticDisturbanceMemory, float* accelerationDisturbanceMemory )
{
    ...
    *magneticDisturbanceGain = ATOFF;
    NEXTRAW
    *accelerationDisturbanceGain = ATOFF;
    NEXTRAW
    *magneticDisturbanceMemory = ATOFF;
    NEXTRAW
...
void VnUartPacket_parseSynchronizationControlRaw ( char *packet, uint8_t* syncInMode, uint8_t* syncInEdge, uint16_t* syncInSkipFactor, uint32_t* reserved1, uint8_t* syncOutMode, uint8_t* syncOutPolarity, uint16_t* syncOutSkipFactor, uint32_t* syncOutPulseWidth, uint32_t* reserved2 )
{
    ...
    *syncInEdge = ATOU8;
    NEXTRAW
    *syncInSkipFactor = ATOU16;
    NEXTRAW
    *reserved1 = ATOU32;
    NEXTRAW
    *syncOutMode = ATOU8;
    NEXTRAW
    *syncOutPolarity = ATOU8;
    NEXTRAW
    *syncOutSkipFactor = ATOU16;
    NEXTRAW
    *syncOutPulseWidth = ATOU32;
    NEXTRAW
    *reserved2 = ATOU32;
}
...
void VnUartPacket_parseVpeMagnetometerBasicTuningRaw ( char *packet, vec3f* baseTuning, vec3f* adaptiveTuning, vec3f* adaptiveFiltering )
{
    ...
    baseTuning->c[0] = ATOFF;
    NEXTRAW
    baseTuning->c[1] = ATOFF;
    NEXTRAW
    baseTuning->c[2] = ATOFF;
    NEXTRAW
    adaptiveTuning->c[0] = ATOFF;
    NEXTRAW
    adaptiveTuning->c[1] = ATOFF;
    NEXTRAW
    adaptiveTuning->c[2] = ATOFF;
    NEXTRAW
    adaptiveFiltering->c[0] = ATOFF;
    NEXTRAW
    adaptiveFiltering->c[1] = ATOFF;
    NEXTRAW
...
void VnUartPacket_parseVpeMagnetometerAdvancedTuningRaw ( char *packet, vec3f* minFiltering, vec3f* maxFiltering, float* maxAdaptRate, float* disturbanceWindow, float* maxTuning )
{
    ...
    minFiltering->c[0] = ATOFF;
    NEXTRAW
    minFiltering->c[1] = ATOFF;
    NEXTRAW
    minFiltering->c[2] = ATOFF;
    NEXTRAW
    maxFiltering->c[0] = ATOFF;
    NEXTRAW
    maxFiltering->c[1] = ATOFF;
    NEXTRAW
    maxFiltering->c[2] = ATOFF;
    NEXTRAW
    *maxAdaptRate = ATOFF;
    NEXTRAW
    *disturbanceWindow = ATOFF;
    NEXTRAW
    *maxTuning = ATOFF;
}
...
void VnUartPacket_parseVpeGyroBasicTuningRaw ( char *packet, vec3f* angularWalkVariance, vec3f* baseTuning, vec3f* adaptiveTuning )
{
    ...
    angularWalkVariance->c[0] = ATOFF;
    NEXTRAW
    angularWalkVariance->c[1] = ATOFF;
    NEXTRAW
    angularWalkVariance->c[2] = ATOFF;
    NEXTRAW
    baseTuning->c[0] = ATOFF;
    NEXTRAW
    baseTuning->c[1] = ATOFF;
...
void VnUartPacket_parseVelocityCompensationStatusRaw ( char *packet, float* x, float* xDot, vec3f* accelOffset, vec3f* omega )
{
    ...
    *xDot = ATOFF;
    NEXTRAW
    accelOffset->c[0] = ATOFF;
    NEXTRAW
    accelOffset->c[1] = ATOFF;
    NEXTRAW
    accelOffset->c[2] = ATOFF;
    NEXTRAW
    omega->c[0] = ATOFF;
    NEXTRAW
    omega->c[1] = ATOFF;
...
void VnUartPacket_parseDeltaThetaAndDeltaVelocityConfigurationRaw ( char *packet, uint8_t* integrationFrame, uint8_t* gyroCompensation, uint8_t* accelCompensation, uint8_t* reserved1, uint16_t* reserved2 )
{
    ...
    *integrationFrame = ATOU8;
    NEXTRAW
    *gyroCompensation = ATOU8;
    NEXTRAW
    *accelCompensation = ATOU8;
    NEXTRAW
    *reserved1 = ATOU8;
    NEXTRAW
    *reserved2 = ATOU16;
...
void VnUartPacket_parseReferenceVectorConfigurationRaw ( char *packet, uint8_t* useMagModel, uint8_t* useGravityModel, uint8_t* resv1, uint8_t* resv2, uint32_t* recalcThreshold, float* year, vec3d* position )
{
    ...
    *useMagModel = ATOU8;
    NEXTRAW
    *useGravityModel = ATOU8;
    NEXTRAW
    *resv1 = ATOU8;
    NEXTRAW
    *resv2 = ATOU8;
    NEXTRAW
    *recalcThreshold = ATOU32;
    NEXTRAW
    *year = ATOFF;
    NEXTRAW
    position->c[0] = ATOFD;
...
void VnUartPacket_parseImuFilteringConfigurationRaw ( char *packet, uint16_t* magWindowSize, uint16_t* accelWindowSize, uint16_t* gyroWindowSize, uint16_t* tempWindowSize, uint16_t* presWindowSize, uint8_t* magFilterMode, uint8_t* accelFilterMode, uint8_t* gyroFilterMode, uint8_t* tempFilterMode, uint8_t* presFilterMode )
{
    ...
    *magWindowSize = ATOU16;
    NEXTRAW
    *accelWindowSize = ATOU16;
    NEXTRAW
    *gyroWindowSize = ATOU16;
    NEXTRAW
    *tempWindowSize = ATOU16;
    NEXTRAW
    *presWindowSize = ATOU16;
    NEXTRAW
    *magFilterMode = ATOU8;
    NEXTRAW
    *accelFilterMode = ATOU8;
    NEXTRAW
    *gyroFilterMode = ATOU8;
    NEXTRAW
    *tempFilterMode = ATOU8;
    NEXTRAW
...
void VnUartPacket_parseGpsCompassBaselineRaw ( char *packet, vec3f* position, vec3f* uncertainty )
{
    ...
    position->c[2] = ATOFF;
    NEXTRAW
    uncertainty->c[0] = ATOFF;
    NEXTRAW
    uncertainty->c[1] = ATOFF;
    NEXTRAW
...
void VnUartPacket_parseYawPitchRollTrueInertialAccelerationAndAngularRatesRaw ( char *packet, vec3f* yawPitchRoll, vec3f* inertialAccel, vec3f* gyro )
{
    ...
    yawPitchRoll->c[2] = ATOFF;
    NEXTRAW
    inertialAccel->c[0] = ATOFF;
    NEXTRAW
    inertialAccel->c[1] = ATOFF;
    NEXTRAW
    inertialAccel->c[2] = ATOFF;
    NEXTRAW
    gyro->c[0] = ATOFF;
    NEXTRAW
    gyro->c[1] = ATOFF;
