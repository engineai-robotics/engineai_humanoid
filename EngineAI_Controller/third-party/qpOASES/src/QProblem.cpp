                                     ...
                                     )
{
    ...
    case PL_DEBUG_ITER:
        ...
        for ( i = 0; i < nV; i++ ) if ( getAbs ( grad[i] ) > stat ) {
                stat = getAbs ( grad[i] );
            }

        /* feasibility */
        for ( i = 0; i < nV; i++ ) if ( lb[i] - x[i] > bfeas ) {
                bfeas = lb[i] - x[i];
            }
        for ( i = 0; i < nV; i++ ) if ( x[i] - ub[i] > bfeas ) {
                bfeas = x[i] - ub[i];
            }
        A->times ( 1, 1.0, x, nV, 0.0, AX, nC );
        for ( i = 0; i < nC; i++ ) if ( lbA[i] - AX[i] > cfeas ) {
                cfeas = lbA[i] - AX[i];
            }
        for ( i = 0; i < nC; i++ ) if ( AX[i] - ubA[i] > cfeas ) {
                cfeas = AX[i] - ubA[i];
            }

        /* complementarity */
        for ( i = 0; i < nV; i++ ) if ( y[i] > +EPS && getAbs ( ( lb[i] - x[i] ) *y[i] ) > bcmpl ) {
                bcmpl = getAbs ( ( lb[i] - x[i] ) *y[i] );
            }
        for ( i = 0; i < nV; i++ ) if ( y[i] < -EPS && getAbs ( ( ub[i] - x[i] ) *y[i] ) > bcmpl ) {
                bcmpl = getAbs ( ( ub[i] - x[i] ) *y[i] );
            }
        for ( i = 0; i < nC; i++ ) if ( y[nV+i] > +EPS && getAbs ( ( lbA[i]-AX[i] ) *y[nV+i] ) > ccmpl ) {
                ccmpl = getAbs ( ( lbA[i]-AX[i] ) *y[nV+i] );
            }
        for ( i = 0; i < nC; i++ ) if ( y[nV+i] < -EPS && getAbs ( ( ubA[i]-AX[i] ) *y[nV+i] ) > ccmpl ) {
